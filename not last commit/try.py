...
class Player(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if pygame.sprite.spritecollideany(self, portal_group):
            global CHANGE_LEVEL
            CHANGE_LEVEL = True
        if pygame.sprite.spritecollideany(self, danger_group):
            HP_STATS -= 1
            self.injured()
...
class Pause_Window(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.frames = []
        self.cut_sheet(pygame.transform.scale(load_image("pause.png"), (4 * 450, 350)), 4, 1,
                       self.frames)
...
class Horizontal_barb(pygame.sprite.Sprite):
    def __init__(self, x, y, lenth):
        ...
        self.image = pygame.transform.scale(load_image("saw/0.gif", -1), (40, 40))
        self.rect = self.image.get_rect()
...
class Die_block(pygame.sprite.Sprite):
    def __init__(self, x, y):
        ...
        self.image = pygame.transform.scale(load_image("fire/" + str(self.cur_frame) + ".png", -1), (50, 100))
        self.rect = self.image.get_rect()
        self.rect.x = x * tile_width
    ...
    while running:
        ...
        if paused:
            if CHANGE_LEVEL:
                ...
                del_level()
                generate_level(load_level("level" + str(CURRENT_LEVEL + 1) + ".txt"))
                CURRENT_LEVEL += 1
                statsBall = Stat_balls()
                statHp = Stat_hp()
                bg = Background()
